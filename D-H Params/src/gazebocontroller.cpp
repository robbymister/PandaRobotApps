...
void GazeboController::moveEEFrameVis(Eigen::Matrix4d ee_pose)
{
	...
	double roll_x = std::atan2(x_rot(2,1),x_rot(2,2));
	double pitch_x = std::atan2(-x_rot(2,0),std::sqrt(x_rot(2,1)*x_rot(2,1)+x_rot(2,2)*x_rot(2,2)));
	double yaw_x = std::atan2(x_rot(1,0),x_rot(0,0));
	
	double roll_y = std::atan2(y_rot(2,1),y_rot(2,2));
	double pitch_y = std::atan2(-y_rot(2,0),std::sqrt(y_rot(2,1)*y_rot(2,1)+y_rot(2,2)*y_rot(2,2)));
	double yaw_y = std::atan2(y_rot(1,0),y_rot(0,0));
	
	double roll_z = std::atan2(z_rot(2,1),z_rot(2,2));
	double pitch_z = std::atan2(-z_rot(2,0),std::sqrt(z_rot(2,1)*z_rot(2,1)+z_rot(2,2)*z_rot(2,2)));
	double yaw_z = std::atan2(z_rot(1,0),z_rot(0,0));
...
void GazeboController::deleteManipulabilitzEllipsoid()
{
	deleteSphere();
}
